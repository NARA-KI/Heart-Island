...
export function currentQuestion(questionBank, state) {
  const questionId = state.orderedQuestionIds[state.currentQuestionIndex];
  return questionBank.questions.find((question) => question.id === questionId);
}
...
export function answerQuestion(questionBank, state, questionId, optionId) {
  ...
  state.currentQuestionIndex = state.orderedQuestionIds.indexOf(questionId);
}

export function goNext(questionBank, state) {
  if (state.currentQuestionIndex < state.orderedQuestionIds.length - 1) {
